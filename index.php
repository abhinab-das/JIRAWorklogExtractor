...
function getData($projectKey,$assignee) {
  global $cfg;
  global $error;
  ...
  $ch = curl_init();
    if ($assignee){
        $url = $cfg['jira_host_address'] . "/rest/api/2/search?jql=assignee=" . $assignee . "%20AND%20project=" . $projectKey . "&maxResults=" . $cfg['max_results'] ;
    }
    else{
        $url = $cfg['jira_host_address'] . "/rest/api/2/search?jql=project=" . $projectKey . "&maxResults=" . $cfg['max_results'] ;
    }
...
if (!empty($_POST)) :
  if ($_POST["submit"] === "fetch") {
    ...
    $result = getData($jiraKey,$assignee);
...
<html lang="en">
  ...
  <body>
    ...
    <form method="POST">
      <div>
        <label>Enter JIRA Project Key<br><input type="text" name="jira_key" placeholder="Eg. PROJ, ABC"></label><br/>
        <label>Enter Assignee Name<br><input type="text" name="assignee" placeholder="Name"></label>
